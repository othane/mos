...
void spis_init(spis_t *spis)
{
	...
	SPI_I2S_ITConfig(spis->channel, SPI_I2S_IT_OVR, ENABLE);
...
static void spis_irq_handler(spis_t *spis)
{
	...
	// check of overflow or error
	if (SPI_I2S_GetFlagStatus(spis->channel, SPI_I2S_FLAG_OVR) == SET)
		sys_nop();
	...
	if ((SPI_I2S_GetFlagStatus(spis->channel, SPI_I2S_FLAG_TXE) == SET) && (spis->channel->CR2 & SPI_CR2_TXEIE))
	{
		volatile union
		{
			uint8_t b[2];
